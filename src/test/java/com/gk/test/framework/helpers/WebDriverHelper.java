...
public abstract class WebDriverHelper extends EventFiringWebDriver {
    ...
    private static WebDriver startChromeDriver() {
        ...
        REAL_DRIVER.manage().window().setSize(BROWSER_WINDOW_SIZE);
        REAL_DRIVER.manage().timeouts().implicitlyWait(18, TimeUnit.SECONDS);
    ...
    public static void HoverOverBurgerMenu(String XpathHover, String XpathClick) {


        WebElement we = REAL_DRIVER.findElement(By.xpath(XpathHover));
        Actions action = new Actions(REAL_DRIVER);
        action.moveToElement(we).moveToElement(REAL_DRIVER.findElement(By.xpath(XpathClick))).click().build().perform();
    ...
    public static void switchToWidgetIframe() throws InterruptedException {


        Thread.sleep(5000);
        REAL_DRIVER.switchTo().frame(REAL_DRIVER.findElement(By.xpath("//iframe[starts-with(@id,'iFrameResizer')]")));

    }


    public static void switchToDefaultIframe() throws InterruptedException {


        Thread.sleep(5000);
        REAL_DRIVER.switchTo().defaultContent();
    }
